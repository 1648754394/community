...
public class MessageController implements CommunityConstant {
    ...
    public String getLetterDetail(@PathVariable("conversationId") String conversationId, Model model, Page page) {
        ...
        User target = getTarget(conversationId, hostHolder.getUser().getId());
    ...
    private User getTarget(String conversationId, int userId) {
        String[] s = conversationId.split("_");
        int id0 = Integer.parseInt(s[0]);
        int id1 = Integer.parseInt(s[1]);
        if(userId == Integer.parseInt(s[0])) {
    ...
    @RequestMapping(path = "/letter/delete")
    @ResponseBody
    ...
    public String getNoticeDetail(@PathVariable("topic") String topic, Model model, Page page) {
        ...
        List<Map<String, Object>> noticeVoList = new ArrayList<>();  //保存用户和帖子
        if(list != null) {
        ...
        //设置已读
        List<Integer> ids = getLettersId(list);
        if (!ids.isEmpty()) {
            messageService.readMessage(ids);
        }
